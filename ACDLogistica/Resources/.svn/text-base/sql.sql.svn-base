<xml>
	...
	<sql>
      <titulo>DGenerarRegMercTransito.cargarDetDifOrden</titulo>
      <descripcion>Cargar los registros con el calculo de los pendientes</descripcion>
      <code>
			Select Art.ARTIC_Codigo
				,Art.ARTIC_Descripcion, TUni.TIPOS_Descripcion As TIPOS_UnidadMedida
      ...
      <code>
			Select * From Entidades As Ent
				...
				Inner Join (
					...
					On CosItem.DOCCO_Codigo = Det.DOCCO_Codigo And CosItem.ENTID_CodigoProveedor = Det.ENTID_CodigoProveedor
      ...
      <code>
			...
			From Logistica.ABAS_DocsCompra As Cab
				...
				Inner Join (
					...
					Where D.ENTID_CodigoProveedor = '{1}' And TIPOS_CodTipoCosteo = '{0}'
					Group by D.DOCCD_Item, DOCCO_Codigo, ENTID_CodigoProveedor
				) As Costeo 
					On Costeo.DOCCO_Codigo = Cab.DOCCO_Codigo And Costeo.DOCCD_Item = Det.DOCCD_Item
   ...
   <sql>
      ...
      <code>
			...
			group By DOCCD_Item,DOCCO_Codigo,ENTID_CodigoProveedor
				...
				<sql>
					<titulo>DManejarStock.IngresoEgreso</titulo>
					<descripcion>Actualizar un Ingreso en la tabla Stocks</descripcion>
					<code>
			Update dbo.Stocks Set STOCK_Cantidad = STOCK_Cantidad + ({0})
				,STOCK_UsrMod = {1}, STOCK_FecMod = '{2}'
			Where
				ARTIC_Codigo = '{3}' And ALMAC_Id = {4}
      </code>
   </sql>
   <sql>
      <titulo>DGenerarRegMercTransito.BusquedaDocsRegCompra</titulo>
      <descripcion>Obtener los documentos que se puede registrar un ingreso de mercadería</descripcion>
      <code>
AND (Select Sum(IsNull(DetOrd.DOCCD_Cantidad, 0) - IsNull(Det.INGCD_Cantidad, 0)) As Cantidad
	From Logistica.ABAS_DocsCompraDetalle As DetOrd
		Left Join (
			Select ARTIC_Codigo, Sum(Det.INGCD_Cantidad) As INGCD_Cantidad
				from Logistica.ABAS_IngresosCompra As Ing
					Inner Join Logistica.ABAS_IngresosCompraDetalle As Det
						On Det.INGCO_Codigo = Ing.INGCO_Codigo
							And Not INGCO_Estado = 'X'
				Where Ing.DOCCO_Codigo = m_abas_docscompra.DOCCO_Codigo
			Group By ARTIC_Codigo
		) As Det 
			On Det.ARTIC_Codigo = DetOrd.ARTIC_Codigo
		Inner Join Articulos As Art
		...
		Inner Join Tipos As TUni
			On TUni.TIPOS_Codigo = Art.TIPOS_CodUnidadMedida
	Where DetOrd.DOCCO_Codigo = m_abas_docscompra.DOCCO_Codigo
	) > 0
	Order By DOCCO_Codigo ASC
      </code>
   </sql>
   <sql>
      <titulo>DGenerarRegMercTransito.BusquedaDocsRegCompraOrden</titulo>
 ...
 AND (Select Sum(IsNull(DetOrd.ORDCD_Cantidad, 0) - IsNull(Det.INGCD_Cantidad, 0)) As Cantidad
	From Logistica.ABAS_OrdenesCompraDetalle As DetOrd
			...
			Select ARTIC_Codigo, Sum(Det.INGCD_Cantidad) As INGCD_Cantidad
				from Logistica.ABAS_IngresosCompra As Ing
					Inner Join Logistica.ABAS_IngresosCompraDetalle As Det
						On Det.INGCO_Codigo = Ing.INGCO_Codigo
							And Not INGCO_Estado = 'X'
				Where Ing.ORDCO_Codigo = m_abas_ordenescompra.ORDCO_Codigo
			Group By ARTIC_Codigo
		) As Det 
			On Det.ARTIC_Codigo = DetOrd.ARTIC_Codigo
		Inner Join Articulos As Art
		...
		Inner Join Tipos As TUni
			On TUni.TIPOS_Codigo = Art.TIPOS_CodUnidadMedida
	Where DetOrd.ORDCO_Codigo = m_abas_ordenescompra.ORDCO_Codigo
	) > 0
	Order By ORDCO_Codigo ASC
      </code>
   </sql>
<sql>
      <titulo>DGenerarDocsCompra.BusquedaDocsRegCompraOrden</titulo>
      <descripcion></descripcion>
      <code>
 AND (Select Sum(IsNull(DetOrd.ORDCD_Cantidad, 0) - IsNull(Det.INGCD_Cantidad, 0)) As Cantidad
	From Logistica.ABAS_OrdenesCompraDetalle As DetOrd
	...
	<sql>
      <titulo>DGenerarDocsCompra.cargarDetDifOrden</titulo>
      <descripcion>Cargar los registros con el calculo de los pendientes</descripcion>
      <code>
			Select Art.ARTIC_Codigo
				,Art.ARTIC_Descripcion, TUni.TIPOS_Descripcion As TIPOS_UnidadMedida
				,IsNull(DetOrd.ORDCD_Cantidad, 0.0) - IsNull(Det.INGCD_Cantidad, 0.0) As CantidadTotal
				,IsNull(Det.INGCD_Cantidad, 0.0) As Entregado
				,IsNull(DetOrd.ORDCD_Cantidad, 0.0) - IsNull(Det.INGCD_Cantidad, 0.0) As DOCCD_Cantidad
				,IsNull(DetOrd.ORDCD_PesoUnitario, 0.0) As DOCCD_PesoUnitario
				,IsNull(DetOrd.ORDCD_PesoUnitario * DetOrd.ORDCD_Cantidad, 0.0) As DOCCD_SubPeso
				,DetOrd.ORDCD_PrecioUnitario As DOCCD_PrecioUnitario
				,ORDCD_PesoUnitario As DOCCD_PrecioUnitario
				,DetOrd.ORDCD_SubImporteCompra As DOCCD_SubImporteCompra
			From Logistica.ABAS_OrdenesCompraDetalle As DetOrd
				Left Join (
					Select ARTIC_Codigo, Sum(Det.INGCD_Cantidad) As INGCD_Cantidad
						from Logistica.ABAS_IngresosCompra As Ing
