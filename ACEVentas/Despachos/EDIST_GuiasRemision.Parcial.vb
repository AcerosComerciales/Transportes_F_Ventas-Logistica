...
#Region " Campos "
    ...
    Private m_guiar_descripcionproveedor As String
    ...
    Private m_pendiente_ingreso As Boolean
    Private m_guiar_impresiones As Integer
    Private m_guiar_ubigeoorigen As String
    Private m_guiar_ubigeodestino As String

    Private m_guiar_paseanulacion As Boolean
    Private m_guiar_usrpaseanulacion As String
    Private m_guiar_fechapaseanulacion As Date

    Private m_entid_codigodestinatario As String
    Private m_guiar_descripciondestinatario As String
    Private m_guiar_tipo As String 'aumentado FRANK 
    Private m_guiar_doccompra As String
...
#Region" Constructores "
	...
	Public Sub New()
		...
		Try
			Dim _obj As Object = ACEVentas.My.Resources.xmlDIST_GuiasRemision
...
#Region" Propiedades "
	...
	Public Property GUIAR_Descripcioncliente() As String
		...
		End Set
	End Property
    Public Property GUIAR_DescripcionProveedor() As String
		Get
			return m_guiar_descripcionproveedor
		End Get
		Set(ByVal value As String)
			If Not IsNothing(m_guiar_descripcionproveedor) Then
				If Not m_guiar_descripcionproveedor.Equals(value) Then
					m_guiar_descripcionproveedor = value
					OnGUIAR_DescripcionProveedorChanged(m_guiar_descripcionproveedor, EventArgs.Empty)
				End If
			Else
				m_guiar_descripcionproveedor = value
				OnGUIAR_DescripcionProveedorChanged(m_guiar_descripcionproveedor, EventArgs.Empty)
	...
	End Property
     ...
     Public Property INGCO_PendIngreso() As Boolean
        Get
            Return m_pendiente_ingreso
        End Get
        Set(ByVal value As Boolean)
            If Not IsNothing(m_pendiente_ingreso) Then
                If Not m_pendiente_ingreso.Equals(value) Then
                    m_pendiente_ingreso = value
                    OnINGCO_PendIngresoChanged(m_pendiente_ingreso, EventArgs.Empty)
                End If
            Else
                m_pendiente_ingreso = value
                OnINGCO_PendIngresoChanged(m_pendiente_ingreso, EventArgs.Empty)
    ...
    ''impresiones
    Public Property GUIAR_Impresiones() As Integer
        Get
            Return m_guiar_impresiones
        End Get
        Set(ByVal value As Integer)
            If Not IsNothing(m_guiar_impresiones) Then
                If Not m_guiar_impresiones.Equals(value) Then
                    m_guiar_impresiones = value
                    OnGUIAR_ImpresionesChanged(m_guiar_impresiones, EventArgs.Empty)
                End If
            Else
                m_guiar_impresiones = value
                OnGUIAR_ImpresionesChanged(m_guiar_impresiones, EventArgs.Empty)
	...
	End Property
		...
		End Set
    End Property
    Public Property GUIAR_UbigeoOrigen() As String
        Get
            Return m_guiar_ubigeoorigen
        End Get
        Set(ByVal value As String)
            If Not IsNothing(m_guiar_ubigeoorigen) Then
                If Not m_guiar_ubigeoorigen.Equals(value) Then
                    m_guiar_ubigeoorigen = value
                    OnGUIAR_UbigeoOrigenChanged(m_guiar_ubigeoorigen, EventArgs.Empty)
                End If
            Else
                m_guiar_ubigeoorigen = value
                OnGUIAR_UbigeoOrigenChanged(m_guiar_ubigeoorigen, EventArgs.Empty)
        ...
        End Set
    End Property
    Public Property GUIAR_UbigeoDestino() As String
        Get
            Return m_guiar_ubigeodestino
        End Get
        Set(ByVal value As String)
            If Not IsNothing(m_guiar_ubigeodestino) Then
                If Not m_guiar_ubigeodestino.Equals(value) Then
                    m_guiar_ubigeodestino = value
                    OnGUIAR_UbigeoDestinoChanged(m_guiar_ubigeodestino, EventArgs.Empty)
                End If
            Else
                m_guiar_ubigeodestino = value
                OnGUIAR_UbigeoDestinoChanged(m_guiar_ubigeodestino, EventArgs.Empty)
    ...
    Public Property GUIAR_PaseAnulacion() As Boolean
        Get
            Return m_guiar_paseanulacion
        End Get
        Set(ByVal value As Boolean)
            If Not m_guiar_paseanulacion.Equals(value) Then
                m_guiar_paseanulacion = value
                OnGUIAR_PaseAnulacionChanged(m_guiar_paseanulacion, EventArgs.Empty)
            End If
        End Set
    End Property
    Public Property GUIAR_DocCompra() As String
        Get
            Return m_guiar_doccompra
        End Get
        Set(ByVal value As String)
            If Not IsNothing(m_guiar_doccompra) Then
                If Not m_guiar_doccompra.Equals(value) Then
                    m_guiar_doccompra = value
                    OnGUIAR_DocCompraChanged(m_guiar_doccompra, EventArgs.Empty)
                End If
            Else
                m_guiar_doccompra = value
                OnGUIAR_DocCompraChanged(m_guiar_doccompra, EventArgs.Empty)
	...
	Public Property GUIAR_FecMod() As Date
		...
		End Set
    End Property
    Public Property GUIAR_UsrPaseAnulacion() As String
        Get
            Return m_guiar_usrpaseanulacion
        End Get
        Set(ByVal value As String)
            m_guiar_usrpaseanulacion = value
        End Set
    End Property

    Public Property GUIAR_FechaPaseAnulacion() As Date
        Get
            Return m_guiar_fechapaseanulacion
        End Get
        Set(ByVal value As Date)
            m_guiar_fechapaseanulacion = value
        End Set
    End Property
    ''sunat
    ...
    Public Property ENTID_CodigoDestinatario() As String
        ...
        End Set
    End Property
    Public Property GUIAR_DescripcionDestinatario() As String
        Get
            Return m_guiar_descripciondestinatario
        End Get
        Set(ByVal value As String)
            m_guiar_descripciondestinatario = value
        End Set
    End Property


    Public Property GUIAR_TipoGuia() As String
        Get
            Return m_guiar_tipo
        End Get
        Set(ByVal value As String)
...
#Region " Eventos "
    ...
    Public Event GUIAR_DescripcionProveedorChanged as EventHandler
    ...
    Public Event INGCO_PendIngresoChanged As EventHandler
    ...
    Public Event GUIAR_ImpresionesChanged As EventHandler
    Public Event GUIAR_UbigeoOrigenChanged As EventHandler
    Public Event GUIAR_UbigeoDestinoChanged As EventHandler
    Public Event GUIAR_PaseAnulacionChanged As EventHandler
    Public Event ENTID_CodigoDestinatarioChanged As EventHandler
    Public Event GUIAR_DescripcionDestinatarioChanged As EventHandler
    Public Event GUIAR_DocCompraChanged As EventHandler
    Public Event GUIAR_TipoGuiaChanged As EventHandler
    ...
    Public Sub OnGUIAR_DescripcionProveedorChanged(ByVal sender As object, ByVal e As EventArgs)
		ActualizarInstancia()
		RaiseEvent GUIAR_DescripcionProveedorChanged(sender, e)
	End Sub
    ...
    Public Sub OnINGCO_PendIngresoChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent INGCO_PendIngresoChanged(sender, e)
    End Sub
	...
	End Sub
    Public Sub OnGUIAR_ImpresionesChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent GUIAR_ImpresionesChanged(sender, e)
    End Sub
    Public Sub OnGUIAR_UbigeoOrigenChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent GUIAR_UbigeoOrigenChanged(sender, e)
    End Sub
    Public Sub OnGUIAR_UbigeoDestinoChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent GUIAR_UbigeoDestinoChanged(sender, e)
    End Sub
    Public Sub OnGUIAR_PaseAnulacionChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent GUIAR_PaseAnulacionChanged(sender, e)
    End Sub
    ...
    End Sub
    Public Sub OnGUIAR_DescripcionDestinatarioChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent GUIAR_DescripcionDestinatarioChanged(sender, e)
    End Sub
    Public Sub OnGUIAR_DocCompraChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
    End Sub

    Public Sub OnGUIAR_TipoGuiaChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
    End Sub
