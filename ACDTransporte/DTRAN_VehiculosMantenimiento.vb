...
#Region " Metodos "
    ...
    ' <returns></returns> 
    ' <remarks></remarks> 
    Public Function TRAN_REPOSS_VehiculoMantenimientos(ByVal x_campo As Short, ByVal x_vehic_placa As String, ByVal m_listtran_vehiculosmantenimiento As List(Of ETRAN_VehiculosMantenimientoDetalle), ByVal x_fecini As DateTime, ByVal x_fecfin As DateTime) As Boolean
        Try
            DAEnterprise.AsignarProcedure("TRAN_REPOSS_VehiculoMantenimientos")
            ...
            DAEnterprise.AgregarParametro("@x_vehic_PLACA", x_vehic_placa, DbType.String, 40)

            'DAEnterprise.AgregarParametro("@VEHIC_Id", x_cadena, DbType.Int16, 10)
            Using reader As DbDataReader = DAEnterprise.ExecuteDataReader()
                If reader.HasRows Then
                    While reader.Read()
                        Dim _tran_vehiculosmantenimientoDetalle As New ETRAN_VehiculosMantenimientoDetalle()
                        ACEsquemas.ACCargarEsquema(reader, _tran_vehiculosmantenimientoDetalle)
                        _tran_vehiculosmantenimientoDetalle.Instanciar(ACEInstancia.Consulta)
                        m_listtran_vehiculosmantenimiento.Add(_tran_vehiculosmantenimientoDetalle)
                    End While
                    Return True
                Else
                    Return False
