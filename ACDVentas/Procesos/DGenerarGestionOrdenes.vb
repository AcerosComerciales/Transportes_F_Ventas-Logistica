...
Public Class DGenerarGestionOrdenes
...
#Region " Metodos "
   ...
   Public Function LOG_DIST_GUIASS_ObtDetDocVenta_Artic(ByVal m_listvent_docsventadetalle As List(Of EVENT_DocsVentaDetalle), ByVal x_docve_codigo As String, ByVal x_almac_id As Short,
                                                        ByVal x_articulo As String, ByVal x_item_doc As Short) As Boolean
      Try
         ...
         DAEnterprise.AgregarParametro("@ARTIC_Item", x_item_doc, DbType.Int16, 2)
         Using reader As DbDataReader = DAEnterprise.ExecuteDataReader()
            If reader.HasRows Then
               While reader.Read()
                  Dim _vent_docsventadetalle As New EVENT_DocsVentaDetalle()
   ...
   End Function
    ...
    ''' 
    ''' </summary>
    ''' <param name="x_almacen"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
   Public Function getNumero(ByVal x_almacen As String) As Integer
      Try
         DAEnterprise.AsignarProcedure(getSelectAll(x_almacen), CommandType.Text)
         Dim m_datos As DataTable = DAEnterprise.ExecuteDataSet().Tables(0)
         If m_datos.Rows.Count > 0 Then
                Return CType(m_datos.Rows(0)("Codigo"), Integer)
         Else
            Return 0
         End If
      Catch ex As Exception
...
#Region " Metodos Adicionales"
    ...
    Private Function getSelectAll(ByVal x_almacen As Long) As String
      Dim sql As String = ""
      Try
         sql &= String.Format(" SELECT Convert(Int, IsNull(Max(GESTOR_Numero), '0'))+1 As Codigo FROM Logistica.ABAS_GestionOrdenes {0}", vbNewLine)
         sql &= String.Format(" Where ALMAC_Id = {0}{1}", x_almacen.ToString, vbNewLine)
         Return sql
      Catch ex As Exception
         Throw ex
      End Try
