...
#Region " Procedimientos Almacenados "
    ...
    ''' <remarks></remarks>
        ...
        Try
            ...
            DAEnterprise.AgregarParametro("@Documento", documento, DbType.String, 50)
            DAEnterprise.AgregarParametro("@GUIA",0,DbType.Boolean)
	...
	End Function
    	...
    	Public Function DIST_ORDENSD_UnReg_Todo(ByVal x_where As Hashtable) As Integer
		Try
			DAEnterprise.AsignarProcedure(getDeletetodo(x_where), CommandType.Text)
			Return DAEnterprise.ExecuteNonQuery()
		Catch ex As Exception
			Throw ex
		End Try
    ...
    Public Function getSerie(ByVal x_campo As String, ByVal x_where As Hashtable) As String
		Try
			DAEnterprise.AsignarProcedure(getSelectSerie(x_campo, x_where), CommandType.Text)
			Dim m_data As DataSet = DAEnterprise.ExecuteDataSet()
    ...
    Public Function getImpresion( ByVal x_where As Hashtable) As Boolean
		Try
			DAEnterprise.AsignarProcedure(getSelectImpresion(x_where), CommandType.Text)
			Using reader As DbDataReader = DAEnterprise.ExecuteDataReader()
				If reader.HasRows Then
					return True
				Else
	...
	#region "Procedimientos Adicionales "
		...
		Private Function getSelectBy(ByVal x_orden_codigo As String) As String
			...
			Try
				sql  = " SELECT * " & vbNewLine
                sql &= " ,(select ENTID_RazonSocial from entidades where entid_codigo=o.ORDEN_UsrMod) as Usuario_Modificador "
				sql &= " FROM Logistica.DIST_Ordenes o" & vbNewLine
				sql &= " WHERE " & vbNewLine
        ...
        Private Function getDeletetodo(ByVal x_where As HashTable) As String 
			Dim sql As String = ""
			Try
                
                sql  = " DELETE FROM HIstorial.DIST_Ordenes" & vbNewLine
				sql &= " WHERE "
				Dim _where As New ACGenerador(Of EDIST_Ordenes)(m_formatofecha) 
				sql &= _where.getWhere(x_where, True) 
        ...
        Private Function getSelectSerie(ByVal x_campo As String, ByVal x_where As HashTable) As String 
			Dim sql As String = ""
			Try
    ...
    Private Function getSelectImpresion( ByVal x_where As HashTable) As String 
			Dim sql As String = ""
			Try

				sql  = String.Format(" SELECT * From Logistica.DIST_Ordenes ")
				sql &= " WHERE "
				Dim _where As New ACGenerador(Of EDIST_Ordenes)(m_formatofecha) 
				sql &= _where.getWhere(x_where, True) 
