...
#Region " Impresoras "
   ...
   Public Shared Sub CargarImpresoras()
      ...
      For index As Integer = 0 To Printers.Count - 1
            ...
            Catch ex As Exception
                ACControles.ACDialogos.ACMostrarMensajeError("Ocurrio un Error revise la Linea A Continuacion : ", ex.Message) 'CAMBIO FRANK 
            End Try
      Next
   End Sub
...
#Region " Tipos "
    ...
    Private Shared m_listDirecciones_ As List(Of EDirecciones)
    ...
    Public Shared ReadOnly Property Tipos() As List(Of ETipos)
        ...
        End Get
    End Property
    Public Shared ReadOnly Property Direcciones_() As List(Of EDirecciones)
        Get
            Return m_listDirecciones_
        End Get
    End Property
   ...
   End Property
    ...
    'funcion TiposPagoFraccion by frank 

    Public Shared ReadOnly Property TipoPagoXFraccionamiento() As List(Of ETipos)
        Get
            Dim _filter As New ACFiltrador(Of ETipos)() With {.ACFiltro = String.Format("TIPOS_Codigo={0}%", ETipos.getCodTipo(ETipos.MyTipos.TransaccionTRecibos))}
   ...
   Public Shared ReadOnly Property TiposDocNotasDebito()
      ...
      End Get
    End Property
    Public Shared ReadOnly Property TiposDocNotasCredito()
        Get
            Dim _filter As New ACFiltrador(Of ETipos)() With {.ACFiltro = String.Format("TIPOS_Codigo={0}%", ETipos.getCodTipo(ETipos.MyTipos.TipoDocComprobante))}
            Dim _filter2 As New ACFiltrador(Of ETipos)() With {.ACFiltro = "TIPOS_Numero=7"}
            _filter.ACFiltrar(m_listTipos)
        ...
        End Get
    End Property
    Public Shared ReadOnly Property TiposMotivoNotaCredito() As List(Of ETipos)
        Get
            Dim _filter As New ACFiltrador(Of ETipos)() With {.ACFiltro = String.Format("TIPOS_Codigo={0}%", ETipos.getCodTipo(ETipos.MyTipos.MotivoNotaCredito))}
            Return _filter.ACFiltrar(m_listTipos)
        End Get
    End Property
   ...
   End Property
    ...
    Public Shared ReadOnly Property TiposDocGastos()
        ...
        End Get
    End Property
    Public Shared ReadOnly Property TiposCombustible()
        Get
            Dim _filter As New ACFiltrador(Of ETipos)() With {.ACFiltro = String.Format("TIPOS_Codigo={0}%", ETipos.getCodTipo(ETipos.MyTipos.TipoConsumo))}
            Dim _filter2 As New ACFiltrador(Of ETipos)() With {.ACFiltro = "TIPOS_Numero=12"}
            _filter.ACFiltrar(m_listTipos)
            Return _filter2.ACFiltrar(_filter.ACListaFiltrada)
        End Get
    End Property

    Public Shared ReadOnly Property TiposUnidad_Toneladas()
        Get
            Dim _filter As New ACFiltrador(Of ETipos)() With {.ACFiltro = String.Format("TIPOS_Codigo={0}%", ETipos.getCodTipo(ETipos.MyTipos.TipoUnidad))}
            Dim _filter2 As New ACFiltrador(Of ETipos)() With {.ACFiltro = "TIPOS_Numero=17"}
            _filter.ACFiltrar(m_listTipos)
            Return _filter2.ACFiltrar(_filter.ACListaFiltrada)
        End Get
    End Property
    Public Shared ReadOnly Property TiposMantenimientosPrev_Corre_Pred()
        Get
            'Filtra en la tabla Tipos solo los que tiene Mant002,03,04 y que esten activos by Frank 
            Dim _filter As New ACFiltrador(Of ETipos)() With {.ACFiltro = String.Format("TIPOS_Codigo={0}%", ETipos.getCodTipo(ETipos.MyTipos.TipoMantenimiento))}
            Dim _filter2 As New ACFiltrador(Of ETipos)() With {.ACFiltro = "TIPOS_Estado=I"}
            _filter.ACFiltrar(m_listTipos)
            Return _filter2.ACFiltrar(_filter.ACListaFiltrada)
        End Get
    End Property
    'Tipos de Direcciones 
    Public Shared ReadOnly Property TiposDireccionesOrigen() 'Prueab funcion Frank 
        Get
            Dim _filter As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("TIPO_Direccion={0}%", "TIPORG")}
            Dim _filter2 As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = "TIPOS_Numero=17"}
            _filter.ACFiltrar(m_listDirecciones_)
            Return _filter2.ACFiltrar(_filter.ACListaFiltrada)
        End Get
    End Property
    Public Shared ReadOnly Property TiposDireccionesOrigen(ByVal x_Lista As List(Of EDirecciones), ByVal x_endid_Codigo As String)
        Get
            Dim _filter As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("TIPO_Direccion={0}%", "TIPORG")}
            Dim _filter2 As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("ENTID_Codigo=%{0}%", x_endid_Codigo)}
            _filter.ACFiltrar(x_Lista)
            Return _filter2.ACFiltrar(_filter.ACListaFiltrada)
        End Get
    End Property
    Public Shared ReadOnly Property TiposDireccionesDestino(ByVal x_Lista As List(Of EDirecciones), ByVal x_entid_codigo As String)
        Get
            Dim _filter As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("TIPO_Direccion={0}%", "TIPDTN")}
            Dim _filter2 As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("ENTID_Codigo={0}%", x_entid_codigo)}
            _filter.ACFiltrar(x_Lista)
            Return _filter2.ACFiltrar(_filter.ACListaFiltrada)
        End Get
    End Property
    Public Shared ReadOnly Property TiposDireccionesXVariableOrigen(ByVal x_Lista As List(Of EDirecciones), ByVal x_entid_codigo As String, ByVal x_variable As String)
        Get
            Dim _filter As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("TIPO_Direccion={0}%", "TIPORG")}
            Dim _filter2 As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("ENTID_Codigo={0}%", x_entid_codigo)}
            Dim _filter3 As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("DIREC_Observacion=%{0}%", x_variable)}
            _filter.ACFiltrar(x_Lista)
            Return _filter3.ACFiltrar(_filter.ACListaFiltrada)
        End Get
    End Property


    Public Shared ReadOnly Property TiposDireccionesXVariableDestino(ByVal x_Lista As List(Of EDirecciones), ByVal x_entid_codigo As String, ByVal x_variable As String)
        Get
            Dim _filter As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("TIPO_Direccion={0}%", "TIPDTN")}
            Dim _filter2 As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("ENTID_Codigo={0}%", x_entid_codigo)}
            Dim _filter3 As New ACFiltrador(Of EDirecciones)() With {.ACFiltro = String.Format("DIREC_Observacion=%{0}%", x_variable)}
    ...
    'Filtramos coleecciones para dispositov movil 


    'Public Shared ReadOnly Property FiltrarTipos_ENTIDADES(ByVal x_Lista As List(Of ETRANGUIAS_Tipos))
    '    Get
    '        Dim _filter As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = String.Format("TipoId={0}%", "7")}
    '        Dim _filter2 As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = String.Format("TipoId={0}%", "8")}
    '        Dim _filter3 As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = String.Format("TipoId=%{0}%", "18")}
    '        _filter.ACFiltrar(x_Lista)
    '        Return _filter.ACFiltrar(_filter.ACListaFiltrada
    '    End Get
    'End Property

    Public Shared ReadOnly Property FiltrarTipos_ENTIDADES(ByVal x_Lista As List(Of ETRANGUIAS_Tipos)) 'Funcion para que los tipos solo muestre 3
        Get
            Dim listaFinal As New List(Of ETRANGUIAS_Tipos)

            ' Filtro 1
            Dim f1 As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = "TipoId = 7"}
            listaFinal.AddRange(f1.ACFiltrar(x_Lista))

            ' Filtro 2
            Dim f2 As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = "TipoId = 8"}
            listaFinal.AddRange(f2.ACFiltrar(x_Lista))

            ' Filtro 3
            Dim f3 As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = "TipoId = 18"}
            listaFinal.AddRange(f3.ACFiltrar(x_Lista))

            'Filtro 4 
            Dim f4 As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = "TipoId = 19"}
            listaFinal.AddRange(f4.ACFiltrar(x_Lista))
    ...
    Public Shared ReadOnly Property FiltrarTipos_RolENTIDADES(ByVal x_Lista As List(Of ETRANGUIAS_Tipos)) 'Rol entidades si es usuario normal o si es usuario adminsitrador papaapapapa
        Get
            Dim listaFinal As New List(Of ETRANGUIAS_Tipos)

            ' Filtro 1
            Dim f1 As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = "TipoId = 1"}
            listaFinal.AddRange(f1.ACFiltrar(x_Lista))

            ' Filtro 2
            Dim f2 As New ACFiltrador(Of ETRANGUIAS_Tipos)() With {.ACFiltro = "TipoId = 2"}
            listaFinal.AddRange(f2.ACFiltrar(x_Lista))
...
#Region "Variables"
    ...
    Private Shared m_listTRANGUIAS_Tipos As List(Of Lista)
...
#Region "Metodos"
    ...
    Public Shared Sub GetTipoPedido()
        ...
        End Try
    End Sub
    Public Shared Sub Cargar_TRANGUIAS_Tipos()
        Try
            Dim managerTRAN_Tipos As New BTRANGUIAS_Tipos()
            managerTRAN_Tipos = New BTRANGUIAS_Tipos
            managerTRAN_Tipos.CargarTodos()
            m_listGUIAS_Tipos = New List(Of ETRANGUIAS_Tipos)(managerTRAN_Tipos.ListTipos)
            m_listGUIAS_Tipos = managerTRAN_Tipos.ListTipos
        Catch
            Throw New Exception("Error: No se han podido obtener los Tipos de Pedidos.")
        End Try
    End Sub
    ...
    Public Shared ReadOnly Property ListGUIAS_Tipos() As List(Of ETRANGUIAS_Tipos)
        Get
            Return m_listGUIAS_Tipos
        End Get
    End Property
...
#Region " UBIGEOS "
    ...
    Private Shared m_listDirecciones As List(Of EDirecciones)


    Private Shared m_listGUIAS_Tipos As List(Of ETRANGUIAS_Tipos)
    ...
    Public Shared ReadOnly Property Distritos() As List(Of EUbigeos)
        ...
        End Get
    End Property
    Public Shared ReadOnly Property Direcciones() As List(Of EDirecciones)
        Get
            Dim m_listDirec As New List(Of EDirecciones)
            For Each Item As EDirecciones In m_listDirecciones
                If Item.UBIGO_Codigo.Trim().Length = 8 Then
                    m_listDirec.Add(Item.Clone)
                End If
            Next
            Return m_listDirec
        End Get
