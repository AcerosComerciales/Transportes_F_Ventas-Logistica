...
Public Class MTipos
#Region " Variables "
   Private managerTipos As BTipos
   Private m_listBindHelper As List(Of ACBindHelper)
   Private m_etipos As ETipos
   Private bs_btipos As BindingSource
...
#Region " Utilitarios "
   ...
   Private Sub formatearGrilla()
      ...
      Try
         ...
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Codigo", "TIPOS_CODTIPO", "TIPOS_CODTIPO", 150, True, False, "System.String", "") : index += 1
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Descripción", "TIPOS_Descripcion", "TIPOS_Descripcion", 150, True, False, "System.String", "") : index += 1
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Desc. Corta", "TIPOS_DescCorta", "TIPOS_DescCorta", 150, True, False, "System.String", "") : index += 1
   ...
   Private Sub cargarCombos()
      Try
         Dim m_listTipos As New List(Of ETipos)(Colecciones.Tipos(ETipos.MyTipos.Tipos))
         Dim _order As New ACOrdenador(Of ETipos)("TIPOS_Descripcion ASC")
         m_listTipos.Sort(_order)
         ACFramework.ACUtilitarios.ACCargaCombo(cmbTipos, m_listTipos, "TIPOS_Descripcion", "TIPOS_Codigo")
      Catch ex As Exception
         Throw ex
      End Try
   End Sub
...
#Region " Cargar Datos "
   ...
   Private Sub cargarDatos()
      Try
         bs_btipos = New BindingSource()
         bs_btipos.DataSource = managerTipos.getListTipos
         c1grdBusqueda.DataSource = bs_btipos
         bnavBusqueda.BindingSource = bs_btipos
      Catch ex As Exception
         Throw ex
      End Try
   End Sub
   ...
   Private Sub AsignarBinding()
      Try
         ...
         'm_listBindHelper.Add(ACBindHelper.ACBind(txtCodigo, "Text", m_etipos, "TIPOS_Codigo"))
         If Not (IsNothing(m_etipos.TIPOS_Codigo)) Then
            txtCodigo.Text = m_etipos.TIPOS_CODTIPO
         End If
         m_listBindHelper.Add(ACBindHelper.ACBind(txtDescripcion, "Text", m_etipos, "TIPOS_Descripcion"))
         m_listBindHelper.Add(ACBindHelper.ACBind(txtDescCorta, "Text", m_etipos, "TIPOS_DescCorta"))
         m_listBindHelper.Add(ACBindHelper.ACBind(actxnNumero, "Text", m_etipos, "TIPOS_Numero"))
      Catch ex As Exception
         Throw ex
      End Try
   End Sub
   ...
   Private Sub cargar()
      Try
         If bs_btipos.Current IsNot Nothing Then
            Dim x_codigo As String = CType(bs_btipos.Current, ETipos).TIPOS_Codigo
            managerTipos.Cargar(x_codigo)
            m_etipos = managerTipos.getTipos()
            ACFramework.ACUtilitarios.ACLimpiaVar(pnlDatos)
            AsignarBinding()
            If (m_etipos.TIPOS_PROTEGIDO) Then
   ...
   Private Function busqueda(ByVal x_cadena As String) As Boolean
      Try
         If managerTipos.Busqueda(x_cadena, cmbTipos.SelectedValue.ToString().Substring(3)) Then
...
#Region "Procesos"
   Private Sub Ordenar(ByVal x_columna As String)
      Dim _ordenador As New ACOrdenador(Of ETipos)
      Try
         If m_order = 2 Then x_columna += " DESC"
         _ordenador.ACOrdenamiento = x_columna
         CType(bs_btipos.DataSource, List(Of ETipos)).Sort(_ordenador)
...
#Region " Tool Bar "
   Private Sub acTool_ACBtnNuevo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles acTool.ACBtnNuevo_Click
      Try
         ...
         m_etipos.Instanciar(ACEInstancia.Nuevo)
         setInstancia(ACFramework.ACUtilitarios.ACSetInstancia.Nuevo)
         AsignarBinding()
         acTool.ACBtnGrabar.Enabled = True
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso Nuevo Tipo", ex)
      End Try
   End Sub
   ...
   Private Sub acTool_ACBtnCancelar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles acTool.ACBtnCancelar_Click
      Try
         ...
         Next
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso Cancelar Tipo", ex)
      End Try
   End Sub
   ...
   Private Sub acTool_ACBtnModificar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles acTool.ACBtnModificar_Click
      Try
         ...
         tabMantenimiento.SelectedTab = tabDatos
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso Modificar Tipo", ex)
      End Try
   End Sub
   ...
   Private Sub acTool_ACBtnGrabar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles acTool.ACBtnGrabar_Click
      ...
      Try
         ...
         If ACFramework.ACUtilitarios.ACDatosOk(pnlDatos, msg) = True Then
            managerTipos.setTipos(m_etipos)
            If (m_etipos.Nuevo) Then m_etipos.TIPOS_Codigo = cmbTipos.SelectedValue.ToString().Substring(3)
                If managerTipos.GuardarSP(GApp.Usuario) Then
                    If m_etipos.Nuevo Then
                        Colecciones.Inicializar(Colecciones.TipoInicializacion.Tipos)
                        'Colecciones.Tipos.Add(m_etipos.Clone)
                    ElseIf m_etipos.Modificado Then
                        Dim _filter As New ACFramework.ACFiltrador(Of ETipos)
                        _filter.ACFiltro = String.Format("TIPOS_Codigo={0}", m_etipos.TIPOS_Codigo)
                        If _filter.ACFiltrar(Colecciones.Tipos).Count > 0 Then
                            _filter.ACListaFiltrada(0) = m_etipos
                        End If
                    End If
   ...
   Private Sub acTool_ACBtnEliminar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles acTool.ACBtnEliminar_Click
      Try
         If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Eliminar Registro: {0}", Convert.ToString(Me.Text)), String.Format("Desea eliminar el registro: {0}?", CType(bs_btipos.Current, ETipos).TIPOS_Descripcion), ACControles.ACDialogos.LabelBotom.Si_No) = DialogResult.Yes Then
            m_etipos = CType(bs_btipos.Current, ETipos)
            m_etipos.Instanciar(ACEInstancia.Eliminado)
            managerTipos.setTipos(m_etipos)
            managerTipos.Guardar(GApp.Usuario)
