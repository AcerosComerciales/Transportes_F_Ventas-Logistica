...
Public Class PGuiaRemiRemitenteFactura
...
#Region " Procesos "
   ...
   Private Function nuevaGuia() As Boolean
      m_etran_guiastransportista = New ETRAN_GuiasTransportista
        Try
            If managerGenerarGuias.cargarDocsCompra(CType(bs_eabas_docscompra.Current, EABAS_DocsCompra).DOCCO_Codigo,
                ...
                managerTRAN_Vehiculos.TRAN_Vehiculos = New ETRAN_Vehiculos
                ...
                If managerEntidades.Cargar(m_etran_guiastransportista.ENTID_Codigo, EEntidades.TipoInicializacion.Direcciones) Then
                    ...
                    If managerEntidades.Entidades.ListDirecciones.Count > 0 Then
                        ...
                        If Not IsNothing(CType(cmbDirOrigen.SelectedItem, EDirecciones).UBIGO_Codigo) Then
                            ...
                            Dim cmb1 As Integer = (Len(cmbOrigenDepartamento.Text) + Len(cmbOrigenProvincia.Text) + Len(cmbOrigenDistrito.Text) + 6) '(Len(cmbDepartamento.Text) + Len(cmbProvincia.Text) + Len(cmbDistrito.Text) + 8)
                            Dim total1 As Integer = Len(cmbDirOrigen.Text)
                            Dim dir1 As Integer = total1 - cmb1

                            cmbDirOrigen.Text = cmbDirOrigen.Text.Substring(0, dir1)
                        Else
                ...
                If Not IsNothing(m_etran_guiastransportista.GTRAN_NroComprobantePago) Then
                    ...
                    txtDVSerie.Text = m_etran_guiastransportista.GTRAN_NroComprobantePago.Substring(2, 4)
                    actxnDVNumero.Text = m_etran_guiastransportista.GTRAN_NroComprobantePago.Substring(6)
                End If
                '' 
                AsignarBinding()
                ...
                actxaRucDestinatario.Text = Trim(GApp.EmpresaRUC)
                ...
                If InStr(cmbDirdestino.Text, cmbDestinoDistrito.Text) Then
                ...
                Else
                    cmbDirDestino.Text = cmbDirDestino.Text
                End If
                txtUbigeoLlegada.Text = GApp.ESucursal.UBIGO_Codigo 'managerEntidades.Entidades.UBIGO_Codigo
        ...
        Catch ex As NullReferenceException
            ACControles.ACDialogos.ACMostrarMensajeInformacion("Aviso", "No hay Documento Seleccionado Para Comenzar ") 'Agregado Frank
        Catch ex As Exception
            Throw ex
        End Try
    End Function
   ...
   Private Function Validar(ByRef msg As String) As Boolean
      ...
      Try
         For Each Item As ETRAN_GuiasTransportistaDetalles In m_etran_guiastransportista.ListETRAN_GuiasTransportistaDetalles
            ...
            _peso += Item.GTDET_Peso * Item.GTDET_Cantidad
         Next

         If managerTRAN_Vehiculos.TRAN_Vehiculos.VEHIC_CargaMax > 0 Then
            If _peso >= managerTRAN_Vehiculos.TRAN_Vehiculos.VEHIC_CargaMax Then
               msg &= String.Format("- El peso excede el limite establecido, {2}Peso Actual: {0} kg.,{2}Peso Establecido: {1} kg.{2}" _
                                    , _peso.ToString(Parametros.GetParametro(EParametros.TipoParametros.pg_FMondo4d)) _
                                    , managerTRAN_Vehiculos.TRAN_Vehiculos.VEHIC_CargaMax.ToString(Parametros.GetParametro(EParametros.TipoParametros.pg_FMondo4d)) _
                                    , vbNewLine)
            End If
         Else
            If _peso >= CType(Parametros.GetParametro("pg_CargaMax", True), Decimal) Then
               msg &= String.Format("- El peso excede el limite establecido, {2}Peso Actual: {0} kg.,{2}Peso Establecido: {1} kg.{2}" _
                                    , _peso.ToString(Parametros.GetParametro(EParametros.TipoParametros.pg_FMondo4d)) _
      ...
      Try
         If Not (bs_tipodocumento.Current Is Nothing) Then
            ...
            managerVENT_PVentDocumento.GetSeries(GApp.Zona, GApp.Sucursal, cmbGuia.SelectedValue)
                ...
                managerVENT_PVentDocumento.ListVENT_PVentDocumento.Reverse() 'ACA SOLO ESTAMOS VOLTEANDO EL ORDEN DE SALIDA DE LOS DATOS 
                For Each item As EVENT_PVentDocumento In managerVENT_PVentDocumento.ListVENT_PVentDocumento '
                    If item.PVDOCU_PredetGuiaRemisRemitTransportista Then
                        _serie = item.PVDOCU_Serie 'aca _Serie es vacio igualamos a item.PVDOCU.Serie  = 18   FRANK 
                        ' MessageBox.Show(item.PVDOCU_Serie)
                    End If
                Next
            ...
            If _serie.Equals("") Then
                ...
                Else : ACFramework.ACUtilitarios.ACCargaCombo(cmbSerie, bs_series, "PVDOCU_Serie", "PVDOCU_Serie", _serie) 'SERIE =18 POR DEFECTO PERO PORQUE ? 
                    ' MessageBox.Show(_serie)
                End If
...
#End Region
   ...
   Private Function GetCampo(ByVal x_ocpion As Short, ByVal x_roles_id As EEntidades.TipoEntidad) As String
      Select Case x_roles_id
         Case EEntidades.TipoEntidad.Transportista
            If x_ocpion = 1 Then
               Return "ENTID_Codigo"
            ElseIf x_ocpion = 2 Then
               Return "ENTID_RazonSocial"
            End If
         Case EEntidades.TipoEntidad.Conductores
            If x_ocpion = 1 Then
   ...
   Private Sub AyudaEntidad(ByVal x_cadenas As String, ByVal x_opcion As Short, ByVal x_roles_id As EEntidades.TipoEntidad)
      Try
         Select Case x_roles_id
            Case EEntidades.TipoEntidad.Transportista
               Dim _where As New Hashtable
               _where.Add(getCampo(x_opcion, x_roles_id), New ACWhere(x_cadenas, ACWhere.TipoWhere._Like))
               If managerEntidades.Ayuda(_where, x_roles_id) Then
               ...
               End If
            Case EEntidades.TipoEntidad.Conductores
               If managerEntidades.AyudaConductores(x_cadenas, x_opcion, x_roles_id) Then
   ...
   Private Sub AyudaVehiculo(ByVal x_cadenas As String, ByVal x_campos As String)
      Try
         If managerTRAN_Vehiculos.Ayuda(x_cadenas, x_campos) Then
            Dim Ayuda As New ACControlesC1.ACAyudaFlex("Buscar Entidad", managerTRAN_Vehiculos.DTTRAN_Vehiculos, False)
            If Ayuda.ShowDialog() = Windows.Forms.DialogResult.OK Then
               managerTRAN_Vehiculos.Cargar(Ayuda.Respuesta.Rows(0)("Interno"))
               actxaCodVehiculo.Text = Ayuda.Respuesta.Rows(0)("Interno")
               txtCargaMax.Text = managerTRAN_Vehiculos.TRAN_Vehiculos.VEHIC_CargaMax.ToString("#,###,##0.0000  Kg.")
               If Ayuda.Respuesta.Rows(0)("Marca Ranfla") Is DBNull.Value Then
                        ...
                        managerGenerarGuias.TRAN_GuiasTransportista.VEHIC_Placa = Ayuda.Respuesta.Rows(0)("Placa")
               Else
   ...
   Private Sub AsignarBinding()
      Try
         ...
         m_listBindHelper.Add(ACBindHelper.ACBind(txtDescProveedor, "Text", m_etran_guiastransportista, "GTRAN_DescEntidadProveedor"))
              ...
              m_listBindHelper.Add(ACBindHelper.ACBind(actxaDescConductor, "Text", m_etran_guiastransportista, "GTRAN_DescConductor"))
              m_listBindHelper.Add(ACBindHelper.ACBind(actxaNroDocConductor, "Text", m_etran_guiastransportista, "ENTID_CodigoConductor"))
            m_listBindHelper.Add(ACBindHelper.ACBind(txtLicencia, "Text", m_etran_guiastransportista, "GTRAN_LicenciaConductor"))
      Catch ex As Exception
         Throw ex
      End Try
   End Sub
...
#Region "Ayudas"
   ...
   Private Sub actxaDescConductor_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaDescConductor.ACAyudaClick
      Try
         AyudaEntidad(actxaDescConductor.Text, 2, EEntidades.TipoEntidad.Conductores)
      Catch ex As Exception
   ...
   Private Sub actxaCodConductor_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaNroDocConductor.ACAyudaClick
      Try
         AyudaEntidad(actxaNroDocConductor.Text, 1, EEntidades.TipoEntidad.Conductores)
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso cargar Conductor", ex)
      End Try
   End Sub
   ...
   Private Sub actxaDescTransportista_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaDescTransportista.ACAyudaClick
      Try
         AyudaEntidad(actxaDescTransportista.Text, 2, EEntidades.TipoEntidad.Transportista)
      Catch ex As Exception
   ...
   Private Sub actxaCodTransportista_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaNroDocTransportista.ACAyudaClick
      Try
         AyudaEntidad(actxaNroDocTransportista.Text, 1, EEntidades.TipoEntidad.Transportista)
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso cargar transportista", ex)
      End Try
   End Sub
      ...
      Try
         ...
         If m_bGenerarGuias.GuiaTransportista(x_gtran_codigo, ETRAN_GuiasTransportista.getEstado(ETRAN_GuiasTransportista.Estado.Anulado)) Then
            ...
            If m_bdetaguia.CargarTodos(_detwhere) Then
               ...
               If IsNothing(m_etran_guiastransportista.GTRAN_CertificadosVehiculo) Then
               ...
               actxaRucDestinatario.Text = Trim(Parametros.GetParametro(EParametros.TipoParametros.Empresa))
         ...
         If ACUtilitarios.ACDatosOk(pnlDatos, msg) And Validar(msg) Then
            If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Grabar Registro: {0}", Me.Text) _
               ...
               m_etran_guiastransportista.ZONAS_Codigo = GApp.Zona
                    ...
                    m_etran_guiastransportista.GTRAN_LicenciaConductor = txtLicencia.Text
                    
                    'm_etran_guiastransportista.GTRAN_DireccionDestinatario = cmbDirDestino.Text
                    m_etran_guiastransportista.GTRAN_DireccionProveedor = cmbDirOrigen.Text & " - " & cmbOrigenDepartamento.Text & " / " & cmbOrigenProvincia.Text & " / " & cmbOrigenDistrito.Text
                    ...
                    m_etran_guiastransportista.VEHIC_Placa = Trim((((m_etran_guiastransportista.VEHIC_Placa).Replace("/", "")).Replace(" ", "")).Replace(".", ""))

                    m_etran_guiastransportista.UBIGO_CodigoProveedor = txtUbigeoPartida.Text.Replace(".", "")
            ...
            If e.KeyData = Keys.Enter Then
                If actxaNroDocConductor.Text.ToString().Length >= ACETransporte.Constantes.LongitudCodigo Then
                    ...
                    If actxaNroDocConductor.ACAyuda.Enabled = True Then
                        ...
                        Else
                            If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Documento de Identidad: {0}", Me.Text) _
                                            , String.Format("El Documento de Identidad {0} no existe, ¿Desea Ingresar la Entidad?", actxaNroDocConductor.Text) _
                                            , ACControles.ACDialogos.LabelBotom.Si_No) = DialogResult.Yes Then
                                NuevaEntidad(actxaNroDocConductor.Text, EEntidades.TipoEntidad.Conductores)
                            Else
...
#End Region
   ...
   End Sub
    ...
    Private Sub tscmbImpresora_DropDown(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tscmbImpresora.DropDown
        ...
        Catch ex As Exception
            Dim MensajeError As String = "Vaya Ocurrio un Error Esto Puede estar Pasando" &
                vbCrLf & "1 . La Impresora No esta Como Predeterminada :" &
                vbCrLf & "2 . La Impresora No esta Instalada -->Recuerda Instalarlo  " &
                vbCrLf & "3 . La PC no Reconoce La Impresora en ese Caso Instale la Impresora De nuevo (Quitar Dispositivo)" &
                vbCrLf & "4 . Ufff Si los Consejos Anteriores no Te sirvieron Entonces Contacte con Sistemas" &
            vbCrLf & "5 . Si Sistemas no Contesta reza 3 padres nuestro y 2 aves marias "

            ACControles.ACDialogos.ACMostrarMensajeError("Vaya Ocurrio un Error", String.Format("{0}", MensajeError, ex.Message))
        End Try
    End Sub

    Private Sub PGuiaRemiRemitenteFactura_Activated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Activated
      btnConsultar_Click(Nothing, Nothing)
   End Sub
    ...
    Private Sub PGuiaRemiRemitenteFactura_Load(sender As Object, e As EventArgs) Handles MyBase.Load
