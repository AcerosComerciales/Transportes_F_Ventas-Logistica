            ...
            If Not IsNothing(managerTRAN_Cotizaciones.TRAN_Cotizaciones.ListTRAN_CotizacionesDetalle) Then
               If managerTRAN_Cotizaciones.TRAN_Cotizaciones.ListTRAN_CotizacionesDetalle.Count > 0 Then
                  ...
                  For Each Item As ETRAN_CotizacionesDetalle In managerTRAN_Cotizaciones.TRAN_Cotizaciones.ListTRAN_CotizacionesDetalle
                     _importeTotal += Item.COTDT_PrecioUnitario * Item.COTDT_Cantidad
                     _pesoTotal += Item.COTDT_PrecioUnitario * Item.COTDT_Cantidad
      ...
      Try
         ...
         For Each Item As ETRAN_CotizacionesDetalle In managerTRAN_Cotizaciones.TRAN_Cotizaciones.ListTRAN_CotizacionesDetalle
            If Not Item.COTDT_Cantidad * Item.COTDT_PrecioUnitario > 0 Then
               m_msg &= String.Format("- El Producto tiene como cantidad {0} y el precio {1}, no es aceptable", Item.COTDT_Cantidad, Item.COTDT_PrecioUnitario, vbNewLine)
            End If
         Next
...
#Region " Utilitarios "
   ...
   Private Sub formatearGrilla()
      ...
      Try
         ...
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdDetalle, index, "Cantidad", "COTDT_Cantidad", "COTDT_Cantidad", 110, True, False, "System.Decimal", Parametros.GetParametro(EParametros.TipoParametros.pg_FMondo3d)) : index += 1
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdDetalle, index, "Precio", "COTDT_PrecioUnitario", "COTDT_PrecioUnitario", 110, True, False, "System.Decimal", Parametros.GetParametro(EParametros.TipoParametros.pg_FMondo3d)) : index += 1
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdDetalle, index, "Sub-Importe", "COTDT_SubImporteVenta", "COTDT_SubImporteVenta", 100, True, False, "System.Decimal", Parametros.GetParametro(EParametros.TipoParametros.pg_FMondo2d)) : index += 1
...
#Region " Metodos de Controles"
   ...
   Private Sub txtGuia_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtGuia.KeyDown
         ...
         If Not IsNothing(managerTRAN_Cotizaciones.TRAN_Cotizaciones) Then
            If Validar(m_msg) Then
               ...
               If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Generar Cotización: {0}", Me.Text) _
                  ...
                  If managerTRAN_Cotizaciones.GenerarCotizacion(GApp.Usuario, m_msg, False) Then
                     '' Imprimir
...
#End Region
   ...
   Private Sub AyudaFletes(ByVal x_cadenas As String)
      Try
         Dim _frm As New CFletes(x_cadenas, CFletes.TAyuda.RazonSocial) With {.WindowState = FormWindowState.Normal, .StartPosition = FormStartPosition.CenterScreen}
         If _frm.ShowDialog() = Windows.Forms.DialogResult.OK Then
            ...
            If managerFletes.Cargar(_join, _where) Then
               ...
               actxnPrecio.Text = Math.Round((managerFletes.TRAN_Fletes.FLETE_MontoPorTM + managerFletes.TRAN_Fletes.FLETE_ImporteIgv) / (1 + Parametros.GetParametro(EParametros.TipoParametros.PIGV) / 100), 2, MidpointRounding.AwayFromZero)
   ...
   Private Sub actxaFleteDesc_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaFleteDesc.ACAyudaClick
      Try
         AyudaFletes(actxaCliRazonSocial.Text)
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), String.Format(Colecciones.getError("00101"), "Cargar Flete"), ex)
      End Try
   End Sub

   Private Sub PCotizaciones_Load(sender As Object, e As EventArgs) Handles MyBase.Load
